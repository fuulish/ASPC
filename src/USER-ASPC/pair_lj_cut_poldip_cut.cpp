...
#include <math.h>
#include <stdlib.h>
#include "pair_lj_cut_poldip_cut.h"
#include "atom.h"
#include "neighbor.h"
...
PairLJCutPoldipCut::PairLJCutPoldipCut(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  compute_efld = 0;
}
...
PairLJCutPoldipCut::~PairLJCutPoldipCut()
{
  if (allocated) {
...
void PairLJCutPoldipCut::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if ( !(compute_efld) && (rsq < cut_coulsq[itype][jtype]) ) {
        ...
        if ( !(compute_efld) ) {
            ...
            // if ( q[j] != 0.0 ) {
                fx = fq*forcecoulx*q[j];
                fy = fq*forcecouly*q[j];
                fz = fq*forcecoulz*q[j];
            ...
            // if ( mu[j][3] != 0.0 ) {
                fx = pre1 * pjdotr * delx - pre2 * mu[j][0];
                fy = pre1 * pjdotr * dely - pre2 * mu[j][1];
                fz = pre1 * pjdotr * delz - pre2 * mu[j][2];
        ...
        if ( (newton_pair || j < nlocal) ) {
          if (compute_efld) {
            ...
            // if ( qtmp != 0.0 ) {
                fx = fq*forcecoulx*qtmp;
                fy = fq*forcecouly*qtmp;
                fz = fq*forcecoulz*qtmp;
            ...
            // if ( mu[i][3] != 0.0 ) {
                fx = pre1 * pidotr * delx - pre2 * mu[i][0];
                fy = pre1 * pidotr * dely - pre2 * mu[i][1];
                fz = pre1 * pidotr * delz - pre2 * mu[i][2];
...
void PairLJCutPoldipCut::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutPoldipCut::settings(int narg, char **arg)
{
  if (narg < 1 || narg > 2)
...
void PairLJCutPoldipCut::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 6)
...
void PairLJCutPoldipCut::init_style()
{
...
double PairLJCutPoldipCut::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutPoldipCut::write_restart(FILE *fp)
{
...
void PairLJCutPoldipCut::read_restart(FILE *fp)
{
...
void PairLJCutPoldipCut::write_restart_settings(FILE *fp)
{
...
void PairLJCutPoldipCut::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
